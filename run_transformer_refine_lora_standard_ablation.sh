...
RUN_ROOT=${RUN_ROOT:-runs/refine_lora_standard_ablation}
RESULTS_CSV=${RESULTS_CSV:-results/refine_lora_standard_ablation.csv}
SEEDS=${SEEDS:-"0 1 2"}
...
usage() {
  cat <<USAGE
Usage:
  $0 run-one <lora_standard> [seed] [target_steps]
...
method_settings() {
  ...
  case "${method}" in
    lora_standard)
      exp_name="lora_standard_seed${seed}"
...
run_one() {
  ...
  echo "=== lora standard ablation method=${method} seed=${seed} target_steps=${target_steps} log_dir=${log_dir} ==="
...
case "$1" in
  ...
  run-seed)
    ...
    run_one lora_standard "${seed}" "${target_steps}"
    ;;
