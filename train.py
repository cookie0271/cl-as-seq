...
def main():
    ...
    # Start distributed training only when multiple GPUs are available
    world_size = torch.cuda.device_count()
    if world_size == 0:
        raise RuntimeError('CUDA is required for train.py. Found 0 CUDA devices.')
...
def train(rank, world_size, port, args, config, distributed=True):
    ...
    set_trainable_modules(
        ...
        train_bitfit=config.get('train_bitfit', config.get('enable_bitfit', False)),
        ...
        bitfit_scope=config.get('bitfit_scope', 'transformer_only'),
        bitfit_include_layernorm_bias=config.get('bitfit_include_layernorm_bias', True),
        bitfit_include_head_bias=config.get('bitfit_include_head_bias', True),
        bitfit_strict_match=config.get('bitfit_strict_match', config.get('strict_match', False)),
    ...
    for step in range(start_step + 1, config['max_train_steps'] + 1):
        ...
        while batch_size - digested > 0:
            ...
            if batch_size - digested - bite == 0:
                ...
                sync_context = model.no_sync() if distributed else nullcontext()
                with sync_context:
                    output = forward_backward(
                        model, train_x_bite, train_y_bite, test_x_bite, test_y_bite,
        ...
        if step % config['summary_interval'] == 0:
            ...
            if rank == 0:
                ...
                if 'mean_gate' in outputs[0]:
                    gate_keys = [
                        'mean_gate', 'std_gate', 'min_gate', 'max_gate',
                        'mean_delta_norm', 'candidate_norm', 'refined_hidden_norm', 'gate_rate_loss'
                    ]
                    for key in gate_keys:
                        if key in outputs[0]:
                            value = sum([output[key] * output['proportion'] for output in outputs if key in output])
                            if distributed:
                                gathered = torch.zeros(world_size, dtype=value.dtype, device=value.device)
                                dist.all_gather_into_tensor(gathered, value.detach())
                                value_item = gathered.mean().item()
                            else:
                                value_item = value.item()
                            writer.add_scalar(f'gating/{key}', value_item, step)
            ...
            if 'attn_losses' in outputs[0]:
                ...
                for layer, attn_loss in enumerate(layer_attn_loss):
                    if distributed:
                        ...
                        attn_value = gathered_attn_loss.mean().item()
                    else:
                        attn_value = attn_loss.item()
                    if rank == 0:
                        writer.add_scalar(f'loss_attn/layer{layer}', attn_value, step)
            ...
            if 'evaluation' in outputs[0]:
                ...
                if distributed:
                    ...
                    acc_train_item = gathered_acc_train.mean().item()
                else:
                    acc_train_item = acc_train.item()
                if rank == 0:
                    writer.add_scalar('acc/train', acc_train_item, step)
            ...
            with torch.no_grad(), Timer('Evaluation time: {:.3f}s'):
                ...
                if total > 0:
                    if distributed:
                        ...
                        acc_test_item = (gathered_correct.sum() / gathered_total.sum()).item()
                    else:
                        acc_test_item = (correct / total).item()
                    if rank == 0:
                        writer.add_scalar('acc/test', acc_test_item, step)
...
def forward_backward(model, train_x, train_y, test_x, test_y, batch_size, config, evaluate=False):
    ...
    if 'gate_rate_loss' in output:
        loss_total = loss_total + config.get('lambda_rate', 0.1) * output['gate_rate_loss']
    ...
    for key in [
        'gate_rate_loss', 'mean_gate', 'std_gate', 'min_gate', 'max_gate',
        'mean_delta_norm', 'candidate_norm', 'refined_hidden_norm'
    ]:
        if key in output:
            detached_output[key] = output[key].detach()
